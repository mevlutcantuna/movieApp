...
function FavouriteButton(props) {
    ...
    useEffect(() => {
        if(props.favourites.includes(props.info)){
    ...
    return (
        <div>
            {index ?
                <button className={'button button__remove'} onClick={() => props.removeFavourite(props.info)}>Remove</button>
                :
                <button className={'button button__add'} onClick={() => props.addFavourite(props.info)}>Add to Favourite</button>}
        </div>
    );
}
...
const mapDispatchToProps = (dispatch) => {
    return{
        ...
        removeFavourite: (item) => dispatch({type: ADDREMOVEFAVOURITE.REMOVE_FAVOURITE,payload:item })
    }
}
